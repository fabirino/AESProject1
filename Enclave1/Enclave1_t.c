...
static sgx_status_t SGX_CDECL sgx_e1_get_sealed_data_size(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_e1_get_sealed_data_size_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_e1_get_sealed_data_size_t* ms = SGX_CAST(ms_e1_get_sealed_data_size_t*, pms);
	ms_e1_get_sealed_data_size_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_e1_get_sealed_data_size_t), ms, sizeof(ms_e1_get_sealed_data_size_t))) {
	...
	_in_retval = e1_get_sealed_data_size(__in_ms.ms_data_size);
...
static sgx_status_t SGX_CDECL sgx_e1_get_unsealed_data_size(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_e1_get_unsealed_data_size_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_e1_get_unsealed_data_size_t* ms = SGX_CAST(ms_e1_get_unsealed_data_size_t*, pms);
	ms_e1_get_unsealed_data_size_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_e1_get_unsealed_data_size_t), ms, sizeof(ms_e1_get_unsealed_data_size_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	...
	_in_retval = e1_get_unsealed_data_size(_in_sealed_data, _tmp_sealed_data_size);
	if (memcpy_verw_s(&ms->ms_retval, sizeof(ms->ms_retval), &_in_retval, sizeof(_in_retval))) {
		status = SGX_ERROR_UNEXPECTED;
		goto err;
	}
...
static sgx_status_t SGX_CDECL sgx_e1_check_credentials(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_e1_check_credentials_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_e1_check_credentials_t* ms = SGX_CAST(ms_e1_check_credentials_t*, pms);
	ms_e1_check_credentials_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_e1_check_credentials_t), ms, sizeof(ms_e1_check_credentials_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	...
	_in_retval = e1_check_credentials(_in_tpdv_data, _in_author, _in_password, _tmp_tpdv_data_size, _tmp_author_len, _tmp_password_len);
	if (memcpy_verw_s(&ms->ms_retval, sizeof(ms->ms_retval), &_in_retval, sizeof(_in_retval))) {
		status = SGX_ERROR_UNEXPECTED;
		goto err;
	}
...
static sgx_status_t SGX_CDECL sgx_e1_create_tpdv(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_e1_create_tpdv_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_e1_create_tpdv_t* ms = SGX_CAST(ms_e1_create_tpdv_t*, pms);
	ms_e1_create_tpdv_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_e1_create_tpdv_t), ms, sizeof(ms_e1_create_tpdv_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	...
	e1_create_tpdv(_in_autor, _in_password, _tmp_author_len, _tmp_password_len, _in_sealed_data, _tmp_sealed_data_size);
	if (_in_sealed_data) {
...
static sgx_status_t SGX_CDECL sgx_e1_add_asset(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_e1_add_asset_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_e1_add_asset_t* ms = SGX_CAST(ms_e1_add_asset_t*, pms);
	ms_e1_add_asset_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_e1_add_asset_t), ms, sizeof(ms_e1_add_asset_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	unsigned char* _tmp_tpdv_data = __in_ms.ms_tpdv_data;
	...
	e1_add_asset(_in_tpdv_data, _in_autor, _in_password, _in_asset_name, _in_asset, __in_ms.ms_tpdv_data_size_unsealed, _tmp_tpdv_data_size, _tmp_author_len, _tmp_password_len, _tmp_asset_name_len, _tmp_asset_size, _in_sealed_data, _tmp_sealed_data_size);
	if (_in_sealed_data) {
		if (memcpy_verw_s(_tmp_sealed_data, _len_sealed_data, _in_sealed_data, _len_sealed_data)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
...
static sgx_status_t SGX_CDECL sgx_e1_list_assets(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_e1_list_assets_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_e1_list_assets_t* ms = SGX_CAST(ms_e1_list_assets_t*, pms);
	ms_e1_list_assets_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_e1_list_assets_t), ms, sizeof(ms_e1_list_assets_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
...
static sgx_status_t SGX_CDECL sgx_e1_extract_asset(void* pms)
{
	...
	//
	sgx_lfence();
	ms_e1_extract_asset_t* ms = SGX_CAST(ms_e1_extract_asset_t*, pms);
	ms_e1_extract_asset_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_e1_extract_asset_t), ms, sizeof(ms_e1_extract_asset_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	unsigned char* _tmp_sealed_data = __in_ms.ms_sealed_data;
...
static sgx_status_t SGX_CDECL sgx_e1_compare_hash(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_e1_compare_hash_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_e1_compare_hash_t* ms = SGX_CAST(ms_e1_compare_hash_t*, pms);
	ms_e1_compare_hash_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_e1_compare_hash_t), ms, sizeof(ms_e1_compare_hash_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	unsigned char* _tmp_sealed_data = __in_ms.ms_sealed_data;
...
static sgx_status_t SGX_CDECL sgx_e1_init_session(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_e1_init_session_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_e1_init_session_t* ms = SGX_CAST(ms_e1_init_session_t*, pms);
	ms_e1_init_session_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_e1_init_session_t), ms, sizeof(ms_e1_init_session_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
...
static sgx_status_t SGX_CDECL sgx_e1_process_message1(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_e1_process_message1_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_e1_process_message1_t* ms = SGX_CAST(ms_e1_process_message1_t*, pms);
	ms_e1_process_message1_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_e1_process_message1_t), ms, sizeof(ms_e1_process_message1_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	...
	e1_process_message1((const sgx_dh_msg1_t*)_in_msg1, _in_msg2, _in_dh_status);
	if (_in_msg2) {
...
static sgx_status_t SGX_CDECL sgx_e1_process_message3(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_e1_process_message3_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_e1_process_message3_t* ms = SGX_CAST(ms_e1_process_message3_t*, pms);
	ms_e1_process_message3_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_e1_process_message3_t), ms, sizeof(ms_e1_process_message3_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	...
	e1_process_message3((const sgx_dh_msg3_t*)_in_msg3, _in_dh_status);
	if (_in_dh_status) {
		if (memcpy_verw_s(_tmp_dh_status, _len_dh_status, _in_dh_status, _len_dh_status)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
...
static sgx_status_t SGX_CDECL sgx_e1_show_secret_key(void* pms)
{
	sgx_status_t status = SGX_SUCCESS;
...
static sgx_status_t SGX_CDECL sgx_e1_get_TPDV_ciphered(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_e1_get_TPDV_ciphered_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_e1_get_TPDV_ciphered_t* ms = SGX_CAST(ms_e1_get_TPDV_ciphered_t*, pms);
	ms_e1_get_TPDV_ciphered_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_e1_get_TPDV_ciphered_t), ms, sizeof(ms_e1_get_TPDV_ciphered_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	unsigned char* _tmp_tpdv_data = __in_ms.ms_tpdv_data;
	...
	size_t _len_tpdv_data = _tmp_tpdv_data_size;
	unsigned char* _in_tpdv_data = NULL;
	unsigned char* _tmp_ciphered_tpdv_data = __in_ms.ms_ciphered_tpdv_data;
	...
	if (_tmp_ciphered_tpdv_data != NULL && _len_ciphered_tpdv_data != 0) {
		if ( _len_ciphered_tpdv_data % sizeof(*_tmp_ciphered_tpdv_data) != 0)
		{
			status = SGX_ERROR_INVALID_PARAMETER;
			goto err;
		}
		if ((_in_ciphered_tpdv_data = (unsigned char*)malloc(_len_ciphered_tpdv_data)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}
...
SGX_EXTERNC const struct {
	...
	15,
	{
		...
		{(void*)(uintptr_t)sgx_e1_check_credentials, 0, 0},
		...
		{(void*)(uintptr_t)sgx_e1_init_session, 0, 0},
		{(void*)(uintptr_t)sgx_e1_process_message1, 0, 0},
		{(void*)(uintptr_t)sgx_e1_process_message3, 0, 0},
		{(void*)(uintptr_t)sgx_e1_show_secret_key, 0, 0},
		{(void*)(uintptr_t)sgx_e1_get_TPDV_ciphered, 0, 0},
	}
};
...
sgx_status_t SGX_CDECL ocall_e1_print_string(const char* str)
{
	...
	ms_ocall_e1_print_string_t* ms = NULL;
	size_t ocalloc_size = sizeof(ms_ocall_e1_print_string_t);
	void *__tmp = NULL;
	...
	ms = (ms_ocall_e1_print_string_t*)__tmp;
	__tmp = (void *)((size_t)__tmp + sizeof(ms_ocall_e1_print_string_t));
	ocalloc_size -= sizeof(ms_ocall_e1_print_string_t);
